...
##########################################
# Scan method (either parallel or unrolled)
# for imagination and observation steps
##########################################
def scan(fn, inputs, start, unroll=True, modify=False):
  fn2 = lambda carry, inp: (fn(carry, inp),) * 2

  # if not unroll, then parallel scan is used
  if not unroll:
    # [1] is used to retrieve final carry value
    return nj.scan(fn2, start, inputs, modify=modify)[1]

  # Get the sequence length by retrieving the leaves of the pytree
  length = len(jax.tree_util.tree_leaves(inputs)[0])

  # Represents structure of the start value and the carry value
  carrydef = jax.tree_util.tree_structure(start)
  carry = start
  outs = []

  # Explicitly iterate through the loop
  for index in range(length):
    carry, out = fn2(carry, jax.tree_util.tree_map(lambda x: x[index], inputs))
    flat, treedef = jax.tree_util.tree_flatten(out)
    assert treedef == carrydef, (treedef, carrydef)
    outs.append(flat)

  # Return all the hidden state outputs as a list of arrays
